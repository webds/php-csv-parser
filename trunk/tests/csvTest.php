...
class csvTest extends PHPUnit_Framework_TestCase
{
    ...
    public function testUses()
    {
        ...
        foreach (fix('non_valid_files') as $file => $msg) {
            $this->assertFalse($this->csv->uses($this->path($file), $msg));
        }

        // oposite
        foreach (fix('valid_files') as $file => $msg) {
            $this->assertTrue($this->csv->uses($this->path($file), $msg));
        }
    }
    ...
    public function testHeaders()
    {
        ...
        $this->csv->uses($this->path('symmetric.csv'));
    ...
    {
        $this->assertTrue($this->csv->uses($this->path('escape_ng.csv')));
    ...
    {
        $this->assertTrue($this->csv->uses($this->path('asymmetric.csv')));
    ...
    {
        $this->assertTrue($this->csv->uses($this->path('symmetric_with_empty_lines.csv')));
        $this->assertEquals(fix('symmetric_connection'), $this->csv->connect());
    } 

    public function test_raw_array_must_remove_empty_records()
    {
        $this->assertTrue($this->csv->uses($this->path('symmetric_with_empty_records.csv')));
        $this->assertEquals(fix('symmetric_connection'), $this->csv->connect());
    } 
    ...
    {
        $this->assertTrue($this->csv->uses($this->path('another_symmetric.csv')));
        $this->assertTrue($this->csv->uses($this->path('symmetric.csv')));
